...
export class CustomWorld {
  setBodyRequest(requestBody: unknown) {
    this.#apiBody = requestBody;
  }
  getApiResult() {
  ...
  #marketTransactions: Record<string, MarketTransaction> = {};
  ...
  saveMarketTransaction(name: string, transactionId: MarketTransaction) {
    this.#marketTransactions[name] = transactionId;
  }

  getMarketTransaction(name: string): MarketTransaction {
    return this.#marketTransactions[name];
  }
  ...
  useContract(chaincodeName: string, contractName: string): void {
    this.getCurrentGateway().useContract(chaincodeName, contractName);
  }
  ...
  async sendPutRequest(path: string, value: string) {
    if (this.#httpClient) {
      this.#apiResponse = await this.#httpClient.put(path, value);
    } else {
      throw new Error("HttpClient not initialized");
    }
  }

  async sendPostRequest(path: string) {
    if (this.#httpClient) {
      this.#apiResponse = await this.#httpClient.post(path, this.#apiBody);
    } else {
      throw new Error("HttpClient not initialized");
    }
  }
  ...
  setArguments(jsonArgs: string, tail: string = ""): void {
