...
const orgs: Record<string, OrgInfo> = {
  ...
  Org2MSP: {
    ...
    cli: "org2_cli",
    peers: ["peer0.org2.example.com:8051"],
  },
  Org3MSP: {
    orgName: "org2.example.com",
    cli: "org3_cli",
...
export class Fabric {
  ...
  generateMeterUser(hsmuserid: string): void {
    const generateOut = execFileSync("./generate-meter-user.sh", [hsmuserid], {
      cwd: fixturesDir,
    });
    console.log(generateOut.toString());
  }

  generatePaymentUser(hsmuserid: string): void {
    const generateOut = execFileSync(
      "./generate-payment-user.sh",
      [hsmuserid],
      {
        cwd: fixturesDir,
      },
